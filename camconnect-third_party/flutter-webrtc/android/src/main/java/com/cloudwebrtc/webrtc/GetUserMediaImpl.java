...
import com.custom.camera.CameraUtils;
...
class GetUserMediaImpl {
    ...
    public void getSupportedCameraResolutions(String trackId, Result result) {
        VideoCapturerInfo info = mVideoCapturers.get(trackId);
        if (info == null) {
            resultError("getSupportedCameraResolutions",
                    "Video capturer not found for id: " + trackId, result);
            return;
        }
        ...
        try {
            ...
            // Most likely the upstream Camera2Capturer class have changed
            resultError("getSupportedCameraResolutions",
                    "[getSupportedCameraResolutions] Failed to get `" + e.fieldName + "` from `" + e.className + "`", result);
            return;
        }

        List<Map<String, Integer>> supportedCameraResolutions;

        try {
            String cameraId = manager.getCameraIdList()[0];
            CameraCharacteristics cameraCharacteristics = manager.getCameraCharacteristics(cameraId);
            supportedCameraResolutions = CameraUtils.getSupportedResolutions(cameraCharacteristics);
        } catch (CameraAccessException e) {
            // Should never happen since we are already accessing the camera
            throw new RuntimeException(e);
        }

        result.success(supportedCameraResolutions);
    }
}
